<template>
    <div>
        <div class="delect_contain_" v-show="showDeletCamera">
            <div class="delect">
                <div class="tips">提示</div>
                <div class="content"><i class="iconfont icon-gantanhao" style="color:rgba(255, 195, 0, 1)"></i> 是否永久删除该摄像头</div>
                <div class="selctType">
                    <button @click="cancel">取消</button>
                    <button>删除</button>
                </div>
            </div>
            <v-mask :isActive="showDeletCamera"></v-mask>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        computed:mapState({
            showDeletCamera:state => state.showDeletCamera
        })
    }
</script>
