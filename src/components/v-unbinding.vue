<template>
    <div>
        <div class="delect_contain_" v-show="showunbinding">
            <div class="delect">
                <div class="tips">提示</div>
                <div class="content"><i class="iconfont icon-gantanhao" style="color:rgba(255, 195, 0, 1)"></i>
                    是否永久解绑{{ deviceName ? deviceName : '设备' }}</div>
                <div class="selctType">
                    <button @click="cancel">取消</button>
                    <button @click="sure">确认</button>
                </div>
            </div>
            <v-mask :isActive="showunbinding"></v-mask>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async sure() {
            if (this.deviceType == '采集器') {
                await this.$store.dispatch('device/unbindingDevice', this.deviceId)
                this.$store.commit('unbinding')
        ...
        getBusData(curDeviceId, curDeviceName){
            this.deviceId = curDeviceId
            this.deviceName = curDeviceName
        }
    },
    computed: mapState({
