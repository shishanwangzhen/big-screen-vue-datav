<template>
    <table class="tableLeft">
        ...
        <tbody>
            <tr v-for="(group, index) in groupArr" :key="group.id" :id="index === curIndex ? 'bgs' : ''"
                @click="getCurindex(group.id, index)">
                <td>
...
export default {
    ...
    methods: {
        async getCurindex(groupId, index) {
            this.getStudentList(groupId)
            this.curIndex = index
        },
        async getStudentList(curGroupId){
