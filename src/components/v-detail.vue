      ...
      <dv-border-box-10 class="dv10">
        <div class="detailContainbox">
          ...
          <ul class="msg">
            ...
            <li>
              设备解码器：
              <ul class="decoderMsg" v-if="getDecoderMsg.length == 0">
                <li>解码器名称：</li>
                <li>解码器路径：</li>
                <li>解码器端口：</li>
                <li>解码器厂商：</li>
              ...
              <ul class="decoderMsg" v-else v-for="item in getDecoderMsg" :key="item.id">
                <li>解码器名称：{{item.name}}</li>
                <li>解码器路径：{{item.path}}</li>
                <li>解码器端口：{{item.port}}</li>
                <li>解码器厂商：{{item.manufacturer}}</li>
                <li>协议类型：{{item.agreement}}</li>
              </ul>
            </li>
            <li>备注：{{ remarks }}</li>
          </ul>
...
export default {
  inject: ["reload"],
  props: ["showDetaileBox", "detailTitle", "model", "deviceId", "remarks", "decoderId"],
  data() {
    return {
  ...
  watch:{
    isActive(newData){
      if(newData && this.decoderId){
        this.getDecoderMsg = this.decoderArr.filter(el => {
          return el.id == this.decoderId
        })
        console.log('getDecoderMsg',this.getDecoderMsg)
      }
    }
  },
  mounted() {
