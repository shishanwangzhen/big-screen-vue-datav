<template>
    <div>
        <div class="delect_contain_" v-if="showDeleteStudent">
            <div class="delect">
                <div class="tips">提示</div>
                <div class="content"><i class="iconfont icon-gantanhao" style="color:rgba(255, 195, 0, 1)"></i> 是否永久删除{{studentName}}</div>
                <div class="selctType">
                    <button @click="cancel">取消</button>
                    <button @click="delect">删除</button>
                </div>
            </div>
            <v-mask :isActive="showDeleteStudent"></v-mask>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            async delect(){
                await this.$store.dispatch('userManagement/updateGroupStudent',this.studentId)
                this.$store.commit('deleteStudent')
        ...
        computed:mapState({
            showDeleteStudent:state => state.showDeleteStudent
        }),
    }
</script>
