...
<template>
  <div>
    <button class="add_device" @click="isActive = !isActive">添加</button>
    <div  >
      <v-addInputList :addTitle="addTitle" :addList="addList" v-if="isActive" @cancel="getRes"></v-addInputList>
...
export default {
  ...
  methods: {
    async decisionActive(){
      await this.$store.commit('isShowAddBox')
      await this.$store.commit('changeAddTitle',this.addTitle)
    },
    cancel() {
